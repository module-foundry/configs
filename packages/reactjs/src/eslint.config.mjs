...
const reactHookGroups = [
  {
    name: "refs",
    selector: "VariableDeclaration:has(CallExpression[callee.name='useRef'])",
  },
  {
    name: "state and custom hooks",
    selector:
      "VariableDeclaration:has(CallExpression[callee.name=/^use(?!(?:Ref|Memo|Callback)$)[A-Z]/])",
  },
  {
    name: "constants",
    selector:
      "VariableDeclaration:not(:has(CallExpression[callee.name=/^use[A-Z]/])):not(:has(VariableDeclarator[init.type=/^(?:ArrowFunctionExpression|FunctionExpression)$/]))",
  },
  {
    name: "memoized values",
    selector:
      "VariableDeclaration:has(CallExpression[callee.name=/^(?:useMemo|useCallback)$/])",
  },
  {
    name: "handlers",
    selector:
  ...
  },
  {
    name: "effects",
    selector:
      "ExpressionStatement:has(> CallExpression[callee.name=/^(?:useInsertionEffect|useLayoutEffect|useEffect|useImperativeHandle)$/])",
  },
];
const reactHookGroupOrder = reactHookGroups.map(({ name }) => name).join(", ");
const reactHookOrderRestrictions = reactHookGroups.flatMap(
  (earlierGroup, earlierIndex) =>
    reactHookGroups.slice(earlierIndex + 1).map((laterGroup) => ({
      selector: `BlockStatement > ${laterGroup.selector} ~ ${earlierGroup.selector}`,
      message: `Keep React component groups in this order: ${reactHookGroupOrder}.`,
    })),
);
const reactEffectSelector = reactHookGroups.at(-1).selector;
const reactHookPaddingRules = reactHookGroups.flatMap(
  (earlierGroup, earlierIndex) =>
    reactHookGroups.slice(earlierIndex + 1).map((laterGroup) => ({
      blankLine: "always",
...
reactHookOrderRestrictions.push({
  selector: `BlockStatement > ${reactEffectSelector} ~ :not(${reactEffectSelector}):not(ReturnStatement)`,
  message: "Keep React effects at the bottom of the component, before return.",
});
...
const frameworkImports = [
  ...
  ["react-ecosystem", "^(?:react-.+|@tanstack/react-.+|@reduxjs/toolkit)$"],
];

const propsRestrictions = [
  "FunctionDeclaration[id.name=/^[A-Z]/][params.0.type='Identifier']:has(JSXElement, JSXFragment)",
  "VariableDeclarator[id.name=/^[A-Z]/] > ArrowFunctionExpression[params.0.type='Identifier']:has(JSXElement, JSXFragment)",
  "VariableDeclarator[id.name=/^[A-Z]/] > FunctionExpression[params.0.type='Identifier']:has(JSXElement, JSXFragment)",
].map((selector) => ({
  selector,
  message: "Destructure React component props in the function signature.",
}));
...
export default tseslint.config(
  ...
  })),
  {
    ...
    files: sourceFiles,
    name: "@module-foundry/eslint/reactjs/regexp",
  },
  {
    plugins,
    files: sourceFiles,
    rules: commonRules,
    name: "@module-foundry/eslint/reactjs/common",
    settings: {
      jsdoc: {
        mode: "typescript",
      },
  ...
  {
    files: frontendFiles,
    name: "@module-foundry/eslint/reactjs/framework",
    languageOptions: {
  ...
  },
  {
    ...
    name: "@module-foundry/eslint/reactjs/component-exports",
    rules: {
      "no-restricted-syntax": [
        "error",
        inlineTypeImportRestriction,
  ...
  },
  {
    ...
    name: "@module-foundry/eslint/reactjs/structural-formatting",
  },
);
